...
export function createHunt(layout:DungeonLayout,normals:readonly Maiden[]) {
  const present=[false,false,false];let lastSpawn=-Infinity;const stalker=createStalker(layout.spawns[0]);
  const active=()=>[...normals.filter((_,i)=>present[i]),...(present[2]&&!stalker.sealed?[stalker.body]:[])];
  return {present,stalker,active,
    ...
    update(time:number,dt:number,player:Point&{readonly yaw:number},tracks:Track[],lure:(p:Point)=>Point|undefined,range:number):{hits:number;spawned:boolean} {
      ...
      if(slot>=0&&time-lastSpawn>=15){
        ...
        if(point){const body=slot===2?stalker.body:normals[slot];body.x=point.x;body.z=point.z;body.yaw=Math.atan2(player.x-point.x,player.z-point.z);body.target={x:player.x,z:player.z};body.mode='chase';if(slot===2)stalker.awakened=true;present[slot]=true;spawned=true;lastSpawn=time;}
      }
