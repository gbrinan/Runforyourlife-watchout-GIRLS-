...
- ✅ 스프라이트 패턴 분석 — docs/ctlc2/SPRITE_PATTERNS.md
- ⏸️ G0 골격 (Autoload 8개, InputMap, 충돌 레이어, 테스트, CI, 라이선스 게이트) — 제목·세계관 확정 → 설계 승인 대기
...
- Q5. 게임 제목·세계관 (사용자가 별도 파일로 제공 예정). 저장소는 gbrinan/Anankad로 확정
...
| 2026-09-25 | 팬게임으로 진행. Konami·원작자의 무료 팬게임 사용 허락을 사용자가 확인 → CTLC2 에셋 업스케일 허용, 근거는 docs/permissions/에 보관 예정 | 사용자 확인 |
| 2026-09-25 | 새 저장소 gbrinan/Anankad(비공개) 생성, Godot 4.7.2 설치 | 사용자 지정 |
| 2026-09-25 | 해상도 정정: 1920×1080 네이티브 렌더 + 2배 에셋(보이는 월드 960×540 원본 px) | 스프라이트 크기 분석 결과 |
| 2026-09-25 | 설계서 승인은 게임 제목·세계관 확정 후 | 사용자 지정 |
...
| 애니메이션에 관계없는 그림이 섞여 보임 | 3 | 프레임 번호가 0부터, 이미지 번호가 1부터임을 확인 → 이미지 = 프레임+1로 보정 |
